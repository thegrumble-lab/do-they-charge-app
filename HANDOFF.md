...
This needs a new environment variable, **`DATABASE_URL`** (the Postgres connection-pooler string from Supabase's Database settings — a real database password, not the anon/service_role JWTs already in use elsewhere), added to Vercel's Environment Variables before this can go live; documented with a placeholder in `.env.local.example`. `npx tsc --noEmit` and `npm run lint` are both clean; a full `npm run build` wasn't run to completion in the sandbox this was built in, since it has no `.env.local` at all (fails at the same pre-existing Supabase env-var check that's always required locally — not a new issue).
...
4. ~~Remaining search latency is a compute-tier ceiling, not a code bug~~ — **superseded.** You upgraded to Supabase Pro (fixing an unrelated egress-quota outage) and the compute tier and Vercel/Supabase region were both since checked and ruled out — see "Full outage (Egress Exceeded)..." above for the fuller diagnosis. The real cause turned out to be Supabase's PostgREST layer itself; the fix is the direct-Postgres bypass described there, which is **built and verified (`tsc`/lint clean) but not yet deployed** — see item 6 below.
5. **Housekeeping**: drop the leftover `public._plan_cache_probe(int)` diagnostic function from the production database.
6. **Two things needed before the PostgREST-bypass search fix can ship**: (a) add a `DATABASE_URL` Vercel environment variable — the Postgres connection-pooler string + real database password, from Supabase's Database settings (see the section above) — and (b) get the code itself from this session onto `main` and deployed, which currently has no working path: the usual device-relay flow (via your laptop) is blocked by an unrelated Windows-update issue, and this session's own sandbox isn't authorized to push directly to the repo. Worth deciding how to get this branch/commit to you — e.g. as a downloadable patch, or by re-authorizing one of the two paths above.
