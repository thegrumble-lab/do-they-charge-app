...
## Search timing out in production again, root-caused three layers deep: bad query plan → statement timeout → crawler-driven CPU throttling (Sept 2026)

A fresh round of `/api/search` 500s ("kings arms berkhamsted" again, among others) turned out to have three separate causes stacked on top of each other. Documenting all three since each one's fix was necessary but not sufficient on its own.

1. **Bad cached query plan on the connection-pooling role.** Supabase's pooled connections all authenticate as a single shared Postgres role, `authenticator` (PgBouncer/Supavisor then switches to `anon`/`authenticated` *inside* the session via `SET ROLE`, but the physical connection's own role-level config is `authenticator`'s). With Postgres's default `plan_cache_mode = auto`, `search_restaurants_ranked()` would get re-planned as a "custom" plan (fast) for the first few calls on a given backend, then Postgres would switch to a "generic" plan (much slower, because it can't use the specific search term's selectivity) once a plan had been reused enough times — and because connections are pooled and reused across unrelated requests, that generic plan then applied to everyone. First attempt at fixing this set `plan_cache_mode` on `anon`/`authenticated` — wrong roles, since those are session-local `SET ROLE` targets, not the physical connection's role. **Fix**: `alter role authenticator set plan_cache_mode = force_custom_plan;` (plus the same on the `postgres` database default, and `pg_terminate_backend()` on existing `authenticator` connections so the new config actually takes effect on fresh connections). Verified via `select unnest(rolconfig) from pg_roles where rolname = 'authenticator'`.
2. **That fix alone didn't stop the 500s.** Re-tests after the plan-cache fix still timed out intermittently. Ruled out plan/index/RLS-policy problems one at a time via direct `EXPLAIN ANALYZE` (as both `postgres`, which bypasses RLS, and `anon`, which enforces it — Supabase's SQL Editor "Run as postgres" bypasses RLS entirely, so any earlier performance test done that way wasn't representative of real traffic): the same query, correctly planned, ranged from ~500ms to 5000ms+ run to run. That variance, plus the project's own Infrastructure page showing CPU usage pinned high, pointed at compute contention rather than anything query-specific. **Stopgap fix, agreed as a deliberate tradeoff** (Supabase's Free plan compute can't be upgraded right now): `alter role authenticator/anon/authenticated set statement_timeout = '20s';` — up from the ~8s default. This traded "some searches 500" for "all searches succeed, some just take longer," which is strictly better for users even though it doesn't fix the underlying slowness.
3. **Even working, search stayed slow (9–11s)** — worth digging into further rather than accepting "at least it works." Two more findings, in order:
   - **`getAreas()` was making ~184 Supabase round-trips per call** (paging through the entire ~184k-row `restaurants` table, 1,000 rows at a time, just to count restaurants per area in JS — PostgREST has no `GROUP BY` over the table API). It's called on every `/browse/[areaSlug]` view that misses the ISR cache, **twice per view** (once in `generateMetadata`, once in the page body — no automatic dedup for a plain async function), plus on `/` and the sitemap routes. `pg_stat_statements` showed two `getAreas()`-shaped query patterns responsible for ~78.7% of *all* database time site-wide over the preceding 5 days. A live Vercel Logs trace nailed it down further: a single crawler (`meta-externalagent`, Meta's bot) hitting `/browse/manchester` triggered **~100 Supabase calls and 41.4 seconds of execution** for that one page view. **Fix**: added a Postgres RPC, `get_area_summaries()` (`select area_slug, min(area), count(*) from restaurants where is_active group by area_slug`, ~650ms via `EXPLAIN ANALYZE` as `anon`), and rewrote `getAreas()` in `src/lib/data.ts` to call it — one round-trip instead of ~184. Shipped via the device-relay flow (this session's cloud sandbox can't push to `origin` directly — patch file → `SendUserFile` → applied and pushed from the repo clone on your laptop, `Downloads/do-they-charge-data/do-they-charge-app`) as commit `21d6751`.
   - **Even after that fix, search RPC calls averaged ~4.2s (max 17s) in `pg_stat_statements`, and a plain `EXPLAIN ANALYZE` run manually (no app involved at all) took 3–7s.** Checked the project's Infrastructure page: the database runs on a **t3a.nano** — a small AWS *burstable* instance, currently at 70% average compute utilization for the day, having spiked from a ~50% baseline. Burstable instances accumulate CPU credits during quiet periods and spend them under load; once credits run low the instance gets throttled regardless of how efficient the query is. That fully explains the run-to-run variance seen throughout this investigation (500ms one run, 5000ms+ the next, same query, same plan, no concurrent queries visible in `pg_stat_activity` at the time) — it's a hard resource ceiling on the Free-tier compute tier, not something further code changes can fix. The `getAreas()` fix should still help going forward: cutting sustained crawler-driven load ~184x means fewer credits get spent on non-search work, which over time should leave more headroom for search itself — but it won't produce instant sub-second responses while the credit balance is still depleted from before the fix landed. The only fix that removes this ceiling entirely is upgrading off the Free-tier shared/burstable compute (e.g. Pro plan's dedicated Micro compute) — deliberately not done this session per your call to hold off on paid upgrades.
4. **Loose end, not yet cleaned up**: a diagnostic function created while investigating step 1 (`public._plan_cache_probe(int)`) is still sitting in the production database and should be dropped.
...
## Full outage (Egress Exceeded), then a deep dive into remaining search latency, then bypassing PostgREST entirely (Sept 2026)

**The outage.** The site went fully down — every page failing to load any data. Cause: Supabase's Fair Use Policy had restricted the project outright for **exceeding the Free plan's 5 GB/month egress quota** (usage had reached 18.4 GB), a harder restriction than the earlier "just slow and sluggish" scare documented above. There's no code fix for a quota — you upgraded to the **Supabase Pro plan ($25/month)** yourself (entering payment details is something I can't do — see the standing rule on handling credentials/payments); the site was back up immediately once that went through.

**Chasing the remaining slowness — three more things tried, all ruled out.** With the outage fixed, search was still taking 3.6–5s per query, so we kept going:
1. **Compute tier**: upgraded the database's compute from Nano to Micro (free — already included in Pro plan credits, no cost change). No measurable improvement (3.7–5.0s after). Ruled out CPU/compute-credit throttling as the cause.
2. **Vercel/Supabase region mismatch**: discovered the Vercel deployment's function region was `iad1` (Washington D.C.) while the Supabase database is in `eu-west-2` (London) — every request was making a genuine transatlantic round trip. Changed Vercel's Function Region setting to `lhr1` (London) and redeployed (confirmed via the `x-vercel-id` response header changing from `lhr1::iad1::...` to `lhr1::lhr1::...`). Still no measurable improvement (3.4–4.3s after). Ruled out network/region latency — the delay was happening somewhere that a same-region hop couldn't fix, which pointed at something structural rather than physical distance.
3. **Query-level and database-level causes, re-checked from scratch**: with the app taken out of the loop entirely (calling Supabase's REST API directly via `fetch`, and running the same queries by hand in the SQL Editor), confirmed the ranked-search query itself only takes 100–300ms (`EXPLAIN ANALYZE`), RLS policies are trivial (`qual: true`, no overhead even when impersonating `anon` directly), `plan_cache_mode` is still correctly set to `force_custom_plan` for every relevant role, prepared-statement/bound-parameter execution is fast when simulated exactly, an exact structural copy of the search function under a different name was equally slow (ruling out anything specific to that one function's catalog history), and there's no connection-pool contention. None of these were the cause.

**What's actually causing it.** With everything above ruled out, the remaining ~2.7–5.1s delay is isolated to **Supabase's PostgREST/API-gateway layer itself, specifically for this RPC's shape of call** — not simple table selects (`/rest/v1/restaurants?select=id&limit=1` etc. are consistently fast, 22–89ms), and not inside Postgres's own query execution (confirmed via `pg_stat_activity`: during the "slow" window of a real request, the backend sits idle — `ClientRead` — rather than doing any work). That's as far as this can be diagnosed without visibility into Supabase's internal PostgREST/Kong logs, which isn't available from the SQL Editor or REST API.

**Options discussed, and what you picked.** Raised a Supabase support ticket as one option; you turned it down — "I don't really understand any of this... you'd be speaking for me" — and asked what else was realistic. Given the choice between a direct-Postgres bypass, having me handle the support ticket myself instead, waiting, or masking it with caching (a poor fit for a ~184k-restaurant site with low query-repeat rates), you picked the bypass: **"Lets try 1."**

**Fix shipped**: added `src/lib/db.ts` — a small `pg` (node-postgres) connection pool, connecting straight to Postgres via Supabase's connection pooler, skipping PostgREST entirely — and rewrote the ranked-search branch of `searchRestaurants()` in `src/lib/data.ts` to use it instead of `supabase.rpc("search_restaurants_ranked", ...)` + a follow-up `.in()` fetch. The new code calls `search_restaurants_ranked()` directly in a single SQL statement that also joins back to `restaurants` and aggregates each restaurant's `reports` via `json_agg` (reimplementing PostgREST's `reports(...)` nested-embed syntax by hand), preserving the function's own ranking order with `row_number() over ()` inside a CTE. Every other function in `data.ts` (browse-mode listing, area pages, individual restaurant lookups, the status-filter allowlist query) is untouched — those were all confirmed fast earlier and stay on the existing `supabase-js`/anon-key client.

This needed a new environment variable, **`DATABASE_URL`**, in Vercel — the Postgres connection string from Supabase's **Connect → Transaction pooler**, with **"Use IPv4 connection" turned on**. That toggle matters: Supabase's dedicated pooler is IPv6-only by default and Vercel's functions can't reach it, so the default string it offers would have failed to connect. The IPv4 (shared pooler) form is `postgresql://postgres.<ref>:<password>@aws-0-eu-west-2.pooler.supabase.com:6543/postgres`. It holds a real database password rather than the publishable anon key. The password can't be retrieved from Supabase after creation (only reset), so this involved resetting it — safe here, because nothing else in the codebase has ever connected directly to Postgres (checked before doing it).

**Result, measured on production after deploying:** searches went from a consistent 3.4-5.0s to a median of ~0.55s (range 99ms-1.5s) — roughly six to ten times faster, with the slowest query now quicker than the fastest one was before. Typo tolerance still works ("wagammma" → 165 Wagamama results in 99ms) and "kings arms berkhamsted" still finds its pub. Homepage, browse pages, restaurant detail pages, status filters, sitemap and robots all verified healthy afterwards.

One deployment note worth remembering: the first deploy of this commit **failed**, because `DATABASE_URL` didn't exist yet and `db.ts` threw at module load — which, since `data.ts` imports it and nearly every page imports `data.ts`, took down the whole build rather than just search. Vercel kept the previous deployment live so the site never went down, but it's a bad failure mode. `db.ts` now creates its pool lazily on first use instead, so a missing or wrong `DATABASE_URL` degrades to "search and sitemaps break" rather than "nothing builds".

## Sitemaps were advertising 3% of the site (Sept 2026)

You'd submitted the site to Search Console and couldn't get it to read the sitemaps. Two separate problems, one of them much more serious than the symptom you noticed.

**1. The sitemaps only contained ~5,000 of ~184,000 restaurant pages.** Every shard was returning exactly 1,000 restaurant URLs — shard 0 had 1,366 entries (3 static + 363 area + 1,000 restaurants) and shards 1-4 had exactly 1,000 each. That "exactly 1,000" is the signature of **PostgREST's row cap, which applies on top of whatever `.range()` asks for**: the shard route requested `RESTAURANTS_PER_SITEMAP` (40,000) rows and silently received 1,000. The XML was perfectly valid and fast to serve, so nothing looked wrong from the outside — it just quietly omitted 97% of the site. Same trap as the one documented at the top of `src/lib/data.ts` (the one that truncated Tower Hamlets at exactly 1,000), in code written after that lesson.

**Fixed** by querying the shard's restaurants through the direct Postgres connection (`getPgPool()`, added for the search fix above), which has no such cap.

**2. Shard size had to shrink at the same time, or the fix would have broken it differently.** At ~153 bytes per `<url>` entry, an uncapped 40,000-URL shard is ~6MB, and **Vercel refuses to return a function response over 4.5MB** — so the "fixed" sitemaps would have failed to serve at all. `RESTAURANTS_PER_SITEMAP` is now **10,000** (~1.5MB per shard, 19 shards for the current dataset), comfortably under both that limit and Google's own 50,000-URL one. The shard count is derived from the same shared helper as the index and robots.txt, so nothing needs updating by hand.
...
3. ~~Minor future optimization, not urgent~~ — **turned out to be a real bug, fixed twice.** First pass switched `getAreas()`'s full-table pagination from `.range()`/OFFSET (O(n²), was tripping statement timeouts) to keyset pagination (`order by id`, `.gt("id", cursor)`) — flat cost regardless of table size, but still ~184 round-trips per call. Second pass (see "Search timing out in production again" above) replaced that whole client-side pagination with a single `get_area_summaries()` Postgres RPC that does the `GROUP BY` server-side — 1 round-trip instead of ~184, confirmed via `pg_stat_statements`. Deployed as commit `21d6751`.
4. ~~Remaining search latency is a compute-tier ceiling, not a code bug~~ — **done.** Root cause turned out to be Supabase's PostgREST layer, not compute or region (both tested and ruled out). Fixed by bypassing it for search; live and measured at ~6-10x faster. See "Full outage (Egress Exceeded)..." above.
5. ~~Drop the leftover `public._plan_cache_probe(int)` diagnostic function~~ — **already gone.** Verified: the database contains exactly four custom functions, all legitimate (`get_area_summaries`, `insert_researched_report`, `search_restaurants_ranked`, `submit_diner_report`). Nothing stray.
6. **Re-submit the sitemaps in Search Console once this deploys**, and check a few days later that they flip from "Couldn't fetch" to Success with a real discovered-page count (expect ~184,000 across 19 shards, not the ~5,000 that the capped version advertised). See "Sitemaps were advertising 3% of the site" above.
