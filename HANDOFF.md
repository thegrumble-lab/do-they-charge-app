...
4. **Remaining search latency is a compute-tier ceiling, not a code bug.** See "Search timing out in production again" above — the database's underlying compute (a burstable t3a.nano) throttles under sustained load regardless of query efficiency. The `getAreas()` fix should ease this over time by cutting sustained load, but a full fix needs a compute upgrade (e.g. Pro plan), which you've said to hold off on for now.
